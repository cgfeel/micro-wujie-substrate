...
| ------------------------------------------------------------------ | ------------------------------------ | ------------ | ---------- |
| `syncUrlToIframe` [[查看](#syncurltoiframe同步主应用路由到子应用)] | 同步路由来自当前路由还是资源入口链接 | 资源入口链接 | 短路径集合 |
...
> `mount` 方法会在同步代码之后在队列中调用，但是执行方式略有差异，见下文
...
- 入口 `script` 注入沙箱后，无论同步还是异步，都会在 `mount` 前绑定 `__WUJIE_MOUNT`
...
> 关于入口 `script` 是内联还是外联，参考：动态加载样式和 `script` [[查看](#7-动态加载样式和-script)]
...
- 综上所述，`mount` 挂载应用似乎只关心 `umd` 初次渲染应用
...
| `umd` 预加载后启动 | 应用实例 `destroy`    | 执行   | 不执行 | 不执行 |
...
- 手动 `destroy` 注销应用，但不建议 [[查看](#其它默认提供的方法)]
...
| `getCssLoader`                                                                           | `getJsLoader`                                                                                                                                        | 唯一的区别是提取插件的属性名，见：通过配置替换资源 [[查看](#通过配置替换资源)]  |
...
- 因此应用中的动态样式也不会重新注入，需要在 `__WUJIE_MOUNT` 前通过 `styleSheetElements` 恢复样式

> `styleSheetElements` 的样式收集来自 3 处 [[查看](#2-stylesheetelements-收集样式表)]
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------- |
| `activeFlag`           | 实例已激活                                                                                                                                                          | `undefined`，通过 `active` 激活 [[查看](#1-更新配置应用信息)]                                    | 通过 `unmount` 失活 [[查看](#1-卸载应用---所有模式)] |
| `bus`                  | 通信对象，使用 `appEventObjMap` 获取事件映射表，通过 `inject` 实现父子应用指向同一个对象 [[查看](#1-inject-注入子应用-3-个对象)]                                    | `EventBus` [[查看](#2-appeventobjmap存储-eventbus-托管的事件)]                                   | `null`                                               |
| `degrade`              | 主动降级，用 `iframe` 作为应用容器                                                                                                                                  | 通过配置文件在构造函数中声明                                                                     | 不处理                                               |
| `elementEventCacheMap` | 当降级时用于保存应用中所有元素事件 [[查看](#elementeventcachemap降级容器事件)]                                                                                      | `WeakMap`，在构造函数中通过 `iframeGenerator` 发起记录 [[查看](#iframegenerator创建沙箱-iframe)] | `null`                                               |
| `execFlag`             | 应用启动状态                                                                                                                                                        | `undefined`，通过 `start` 激活 [[查看](#5-队列前的准备)]                                         | `null`                                               |
| `execQueue`            | `start` 应用中的任务队列 [[查看](#1-execqueue-应用启动执行队列)]                                                                                                    | `[]`                                                                                             | `null`                                               |
| `id`                   | 应用名列                                                                                                                                                            | `name`，字符类型                                                                                 | 不处理                                               |
| `mountFlag`            | `umd` 模式挂载应用                                                                                                                                                  | `undefined`，应用 `mount` 后为 `true`，`unmount` 后为 `false`                                    | `null`                                               |
...
> 顺序从上至下，只列举了和 `url` 直接关联的方法，不包含衍生对象，例如：`proxyLocation` [[查看](#wujie-中的代理)]
...
通过 `getTargetValue` 获取沙箱 `window` 的属性 [[查看](#gettargetvalue-从对象中获取属性)]：
...
- 全局 `window` 描述信息中不存在 `get` 属性，从沙箱中获取属性 `window`
...
- 从全局 `window` 上获取：`document`，从应用实例上获取：`shadowRoot` 容器、`proxyLocation`
- 从沙箱 `window` 上获取原生方法：`rawCreateElement` 创建元素、`rawCreateTextNode` 创建文本

> 在获取对象前需要确保 `shadowRoot` 已实例化，否则通过 `stopMainAppRun` 输出警告并抛出错误终断执行
...
| 条件                                               | 通常做法                                                                                                                                                                 |
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 只替换带有子应用 `origin` 的绝对路径               | 通常使用相对路径，毕竟线上线下 `host` 不一样，但相对路径也存在问题                                                                                                       |
| 只拦截 `location.replace` 不拦截 `history.replace` | 对于单例应用来说通常是由 `history` 来负责路由 `replace`，这个操作在沙箱初始化时由 `patchIframeHistory` 做了拦截 [[查看](#patchiframehistory-劫持沙箱-iframe-的-history)] |
...
| `proxyDocument` [[查看](#2-代理空对象作为-proxydocument)]                       | 通用   | 代理中属性 `documentURI`、`URL` 的指向     |
| `proxyWindow` [[查看](#1-代理沙箱-window-作为-proxywindow)]                     | 非降级 | 代理中属性 `location` 的指向               |
...
通过 `getOwnPropertyNames` 打补丁，见：源码 [[查看](https://github.com/Tencent/wujie/blob/9733864b0b5e27d41a2dc9fac216e62043273dd3/packages/wujie-core/src/iframe.ts#L231)]
...
- 根据当前应用名 `id` 将值更新为 `encodeURIComponent` 的 `url`
...
- `isCrossOriginScript`：提取的 `script` 存在跨域行为 `crossorigin`
- `crossOriginType`：跨域类型，只取 `anonymous` 不发送凭据，`use-credentials` 发送凭据，否则为空字符
...
- `rewriteAppendOrInsertChild`：处理应用中动态加载的 `script` [[查看](#rewriteappendorinsertchild重写-appendchild-和-insertbefore)]
...
- 应用中动态添加的 `script`，不收集元素 `ignore` 属性，除了 `jsIgnores` 都加载为内联 `script`
...
- 而在调用场景中通过 `await` 可以确保执行前优先发起任务加载 `script`
...
| `src` 为元素       | 包含标签 `<>` | 提取元素中的样式，以 `Promise` 返回内联代码                                  |
...
- `rewriteAppendOrInsertChild`：处理应用中动态加载的外联样式 [[查看](#rewriteappendorinsertchild重写-appendchild-和-insertbefore)]
- `processCssLoaderForTemplate`：通过配置手动注入样式到应用头部和尾部 [[查看](#processcssloaderfortemplate手动添加样式)]

> 应用中动态加载的内联样式不需要调用 `getExternalStyleSheets`；在单例应用，如 `React` 中通常会通过入口文件动态加载样式，以内联的方式将代码注入样式，见：加载流程 [[查看](https://github.com/cgfeel/zf-micro-app/blob/main/doc/wujie-umd-patch_css_rules.md)]
...
- 而在调用场景中通过 `await` 可以确保执行前优先发起任务加载样式
...
> 应用内动态添加的样式和 `script`，会先注入静态 `script` 到沙箱后，然后再拦截动态添加的样式和 `script`，在单例应用中通常将入口 `script` 注入后，进行动态拦截
...
- 不存在 `LOADING_DATA_FLAG` 节点的情况下，挂载前需要通过 `clearChild` 清空容器
...
- `iframe.ts` - `recoverDocumentListeners` [[查看](https://github.com/Tencent/wujie/blob/9733864b0b5e27d41a2dc9fac216e62043273dd3/packages/wujie-core/src/iframe.ts#L348)]
...
> 监听事件中的参数 `options` 仅用于发起监听，不在记录中缓存，因为记录事件的目的是为了卸载应用时删除对应事件
...
- 使用 `handle` 从集合中获取回调对象 `callback`，不存在则通过 `bind` 修正后记录 `handle`
...
- 否则判断集合中是否包含事件类型，不包含则插入后更新记录，包含则不做任何操作
...
> 以上示例中，当拦截事件后监听对象从沙箱 `document` 转变为容器 `document`，但是监听事件的回调函数中 `this` 仍旧为沙箱 `document`
...
> `baseURI` 为子应用 `origin` + `pathname`，见：`patchElementEffect` [[查看](#patchelementeffect为元素打补丁)]
...
- `rawElement`：动态添加的 `script` 元素，用于 `setTagToScript` 打标记，可选参数 [[查看](#为动态添加的-script-打标记)]
...
> `rewriteAppendOrInsertChild` 来自渲染资源到容器时调用 `patchRenderEffect` [[查看](#patchrendereffect-为容器打补丁)]

整个函数围绕 2 个对象展开：

- `scriptElement`：根据提供的对象，创建 `script` 元素插入沙箱 `iframe`
- `nextScriptElement`：执行完毕后插入到沙箱，用于提取并执行下个队列，见：`start` [[查看](#-start-启动应用)]
...
> `degrade` 和 `module` 决定是否包裹模块，而打包方式不同将决定应用入口 `script` 的上下文 [[查看](#7-动态加载样式和-script)]
...
| 提供 `html` 或使用缓存                      | 没有静态样式                                                  | 通过 `await` 确保完成                  |
...
- 将会在 `active` 中通过 `await this.iframeReady` 确保注入资源前沙箱已完成初始化
...
- 由于 `iframeGenerator` 设置沙箱和基座同域 [[查看](#iframegenerator创建沙箱-iframe)]
- 所以需要在沙箱中通过 `base` 元素，基于应用链接，修正容器中所有资源的相对路径
...
- 通过 `anchorElementGenerator` 创建 2 个链接对象：基座 `origin`，子应用入口链接 [[查看](#anchorelementgenerator转换-url)]
- 使用子应用 `origin` + 基座 `pathname` 作为 `base` 元素的 `href`，之后插入沙箱 `head` 中

> 初始化沙箱时链接为 `mainHostPath`：基座 `origin`，而 `pathname` 通常为 `/`（随实际情况改变）
...
- 从应用实例中获取：`id`、`url`、`sync`，`execFlag`、`prefix`，用于计算子应用路由
- 从应用实例中获取：`inject` 得到基座 `origin`，用于为沙箱 `iframe` 更新 `history`

同步路由到子应用最终目的，以资源入口链接作为初始路由：

- `preAppRoutePath` 当前沙箱路由：`pathname` + `search` + `hash`
- `syncUrl` 计算后得出的路由：计算方式继续往下看
...
| 返回类型         | 同步的链接       | 说明                                                                                   |
| ---------------- | ---------------- | -------------------------------------------------------------------------------------- |
| `pathname`       | 匹配的子应用路由 | 正确匹配路由                                                                           |
| 绝对路径的 `url` | 资源入口链接     | 劫持链接做路由，见 `locationHrefSet` [[查看](#locationhrefset拦截子应用-locationhref)] |
...
| ------------ | -------- | ------------ | ------------------------------------ |
| `alive` 模式 | ✅       | ✅           | 沙箱不销毁，路由没有变化无需同步     |
| `umd` 模式   | ✅       | ✅           | 沙箱不销毁，使用资源入口链接无需同步 |
| 重建模式     | ✅       | ✅           | 不存在再次加载                       |

> 首次加载包括：初次 `startApp`、`preloadApp`
...
| -------- | ---------- | ------------------------------------------------------ | ------ | ---------------------------------------------- |
| 存在     | 已启动     | 通过 `unmount` 已失活 [[查看](#1-卸载应用---所有模式)] | 已配置 | 并非 `hrefFlag` 劫持链接 [[查看](#2-特殊属性)] |
...
- 将短路径从 `prefix` 找到对应的完整路径，替换后返回

> 若因 `queryMap` 不存在应用名，或因无法通过正则匹配得到 `undefined`，同样返回空字符
...
| 获取需要同步的路由             | `getSyncUrl`                                                       | 找不到 `search`，返回空字符                                                               |
...
| `url`                 | `base`                | 结果                       |
| --------------------- | --------------------- | -------------------------- |
| `http` 开头的绝对路径 | --                    | `base` 被忽略              |
| 相对路径              | 相对路径              | 报错                       |
| 相对路径              | `http` 开头的绝对路径 | 有效，基于 `base` 拼接链接 |

> 以上情况可参考 `new URL` 特性 [[查看](#new-url处理并返回-url-对象)]

直接返回参数 `url` 有 2 个情况：

- `url` 是空字符，这点和 `new URL` 特性是不一致的，`URL` 对象此时会返回 `base`
...
- 只有 `entry` 是字符类型的时候才可以作为相对路径，并且需要同时提供 `base`
...
参考 `new URL` 会保留 3 个特性 [[查看](#new-url处理并返回-url-对象)]
...
| `http` 开头绝对路径 | 基本是                               |
| 以 `/` 开头相对路径 | 不建议，将基座链接作为子应用资源目录 |
| 空字符              | 不建议，将基座链接作为子应用资源目录 |

`entry` 为 `URL` 对象存在的问题：

- 应返获取 `url.href` 进行操作，同时保留对其他 `object` 的判断返回 `/`
...
**特性 4：`base` 作为拼接时，将丢弃部分属性 [[查看](#new-url处理并返回-url-对象)]**
...
> `_patcher` 属性由 `handleStylesheetElementPatch` 添加到 `style` 元素

不处理的情况：

- `degrade` 降级：没有 `shadowRoot`，`iframe` 容器也不存在兼容样式的问题
- 设置 `cssIgnores` 通过浏览器添加的外联样式，只能对内联样式打补丁
...
| 注入方式 | 分类     | 加载方式                                    | 如何打补丁                                                                       |
| -------- | -------- | ------------------------------------------- | -------------------------------------------------------------------------------- |
| 动态添加 | 外联样式 | 加载后作为内联样式添加到容器                | `handleStylesheetElementPatch`                                                   |
| 动态添加 | 外联样式 | 配置 `cssIgnores`，作为浏览器加载的外联样式 | 不打补丁                                                                         |
| 动态添加 | 内联样式 | 由单页应用创建空的动态样式                  | `patchStylesheetElement` [[查看](#patchstylesheetelement劫持处理样式元素的属性)] |
| 动态添加 | 内联样式 | 原生方法动态添加的样式                      | `handleStylesheetElementPatch`                                                   |
| 静态样式 | 所有     | 通过 `importHTML` 提取，不包含任何 `ignore` | `patchCssRules` [[查看](#-patchcssrules-子应用样式打补丁)]                       |
| 静态样式 | 所有     | 元素包含任何 `ignore` 属性                  | 被注释代替                                                                       |
| 静态样式 | 外联样式 | 配置 `cssIgnores`，作为浏览器加载的外联样式 | 不打补丁                                                                         |
