...
表格中所在 `window` 列中，将作为 `processAppForHrefJump` 监听事件：
...
- 但应用内的路由更新会通过 `syncIframeUrlToWindow` 更新基座路由 [[查看](#synciframeurltowindow-监听沙箱前进后退)]
...
- 不会回退到上一页面，而是上一个应用的入口页，在使用上就会有一种割裂感
- 比如应用是 `react` 以劫持容器渲染，当点击基座链接切出应用后，是无法通过后退还原的
...
| 条件                    | 原因                                                                                   |
| ----------------------- | -------------------------------------------------------------------------------------- |
| `queryMap` 为应用内路由 | ① `hrefFlag` 不可能为 `true`，② 路由也不是被劫持的 `http` 开头                         |
| `herfFlag` 为 `true`    | 前进优先匹配 `queryMap`，被 `locationHrefSet` 劫持后不会继续劫持，`history` 只存在后退 |
