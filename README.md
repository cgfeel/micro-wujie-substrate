...
| 分类     | 加载方式                                    | 如何打补丁                                                 |
| -------- | ------------------------------------------- | ---------------------------------------------------------- |
| 所有     | 通过 `importHTML` 提取，不包含任何 `ignore` | `patchCssRules` [[查看](#-patchcssrules-子应用样式打补丁)] |
| 所有     | 元素包含任何 `ignore` 属性                  | 被注释代替                                                 |
| 外联样式 | 配置 `cssIgnores`，作为浏览器加载的外联样式 | 不打补丁                                                   |
...
- 通过 `setTagToScript` 为动态添加的 `script` 元素打标记 [[查看](#为动态添加的-script-打标记)]
...
> `postProcessTemplate` 可能是工作人员为后续更新留下的一个口子
...
| `umd` 模式切换        | 仅用于替换手动加载的外联样式 | 用于恢复容器样式                   |
...
- `execQueue`：首次启动会执行收集提取队列，再次启动仅重建模式需要重新队列并执行注入
...
- 但是调用场景都来自启动应用或应用预执行，见：资源缓存集合 [[查看](#2-资源缓存集合)]
