...
| 模式 ④：通过 `afterScriptResultList` 异步插入队列的 `script`  | 启动前通过 `js-after-loader`配置，当集合中有一个队列带有 `async` 的 `script`   | 和模式 ③ 一样，区别在于 `afterScriptResultList` 在执行前，应用已完成加载，中断的只有 `loaded` 事件                                                       |
