...
| -------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| 收集方法 | `fetchAssets` [[查看](#fetchassets加载资源缓存后返回-promise)] | `rewriteAppendOrInsertChild` [[查看](#rewriteappendorinsertchild重写-appendchild-和-insertbefore)] | `patchCssRules` [[查看](#-patchcssrules-子应用样式打补丁)] |
| 用处     | 处理请求，记录缓存                                             | 动态添加样式                                                                                       | 打补丁                                                     |
...
| --------------------- | -------------------------------------- | ------------------------------------ |
| 初次启动应用          | 缓存静态资源中的外联样式，替换入口资源 | 用于收集样式元素，不参与渲染         |
| 预加载&预执行         | 预加载缓存静态外联样式，替换入口资源   | 预执行收集动态样式，每次渲染收集补丁 |
| `active` 预加载后启动 | 不使用                                 | 不使用                               |
| `active` 模式切换     | 不使用                                 | 不使用                               |
| `umd` 模式切换        | 使用 `template`，不需要替换样式        | 用于恢复容器样式                     |
| 重建模式切换          | 使用缓存的外联静态样式，替换入口资源   | 重新记录，不参与渲染                 |
...
| `active` 预加载后启动 | 存在则使用           | 提取但不加载，也不使用               | 不使用               | 不使用                                             |
