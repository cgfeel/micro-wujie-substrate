...
| `connectedCallback`        | 声明组件自增变编号添加到映射表、发起应用挂载                                                                                                                                                      | 根据 `name` 属性拿到实例：为沙箱 `iframeWinow` 打补丁、将当前组建指向实例的 `shadowRoot`                                     |
| `disconnectedCallback`     | 用组件编号从映射表中下线、发起应用卸载                                                                                                                                                            | 根据 `name` 属性拿到实例并发起卸载                                                                                           |
| 自定义应用更新             | 开发人员只能配置规则，不能决定如何更新                                                                                                                                                            | 规则交由开发人员自己决定，一旦更新应用就一定是重新渲染                                                                       |
| 生命周期                   | `connectedCallback`、`disconnectedCallback`、`attributeChangedCallback`，其中 `connectedCallback` 会触发 `created`                                                                                | `connectedCallback`、`disconnectedCallback`，组件的生命周期没有对应应用的生命周期                                            |
