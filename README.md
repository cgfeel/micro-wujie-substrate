...
| 手动 `start`   | 不支持手动启动，资源加载完成后调用 `mount` 挂载 [[查看](https://github.com/cgfeel/micro-app-substrate?tab=readme-ov-file#32-mount-%E6%8C%82%E8%BD%BD%E5%BA%94%E7%94%A8)] | `startApp` 或 `preloadApp` 时调用应用 `start` 方法                                                                        |
