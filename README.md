...
| `provide`              | 为子应用提供通信 `bus`、传递数据 `props`、获取 `shadowRoot` （可选）和子应用的 `location`，见：文档 [[查看](https://wujie-micro.github.io/doc/api/wujie.html#wujie)] | 在构造函数里提供 `bus`、`location`，在 `active` 中提供 `props` 和 `shadowRoot` | `null`                  |
