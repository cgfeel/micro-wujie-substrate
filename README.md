...
| 映射表         | `appInstanceMap` 应用实例映射表，和组件映射表不同                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `idToSandboxCacheMap` 实例映射表，组件挂载时通过应用名从映射表获取实例 [[查看](#1-idtosandboxcachemap存储无界实例和配置)] |
...
| 手动 `start`   | 不支持手动启动                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `startApp` 或 `preloadApp` 时调用应用 `start` 方法                                                                        |
| `mount` 应用   | 自动：由组件或资源加载完毕决定，在 `mount` 中会 `start` 沙箱 [[查看](https://github.com/cgfeel/micro-app-substrate?tab=readme-ov-file#32-mount-%E6%8C%82%E8%BD%BD%E5%BA%94%E7%94%A8)]                                                                                                                                                                                                                                                                                                                                                | 不支持外部调用，由 `start` 方法通过队列执行                                                                               |
